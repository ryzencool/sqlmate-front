        ...
        <div className={"flex flex-col gap-3"}>
            <div className={"flex flex-col gap-2"}>
                <div className={"flex flex-row gap-3 items-end"}>
                    {/* @ts-expect-error TS(2339) FIXME: Property 'name' does not exist on type 'number'. */}
                    <div className={"text-xl font-bold"}>{activeTableId.name}</div>
                    {/* @ts-expect-error TS(2339) FIXME: Property 'note' does not exist on type 'number'. */}
                    <div className={"text-base"}>{activeTableId.note}</div>
                </div>
                <div className={'text-sm text-slate-400'}>
                    共<span className={'text-base text-black p-1'}>{fakerData.length}</span>条数据
                </div>
                {/* @ts-expect-error TS(2741) FIXME: Property 'getSelectedRows' is missing in type '{ d... Remove this comment to see the full error message */}
